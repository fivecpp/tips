...
采用的是博通BCM2835。这个片上系统包含一块支持硬件浮点的ARM1176JZFS ARM CPU核心，其运行频率为700MHz，和一个Videocore 4显示核心(GPU)。GPU支持使用H.264解码器，进行蓝光质量的视频播放，数据速率为40MBit/s。同时还包含一个3D核心，可以使用 OpenGL ES2.0和OpenVG库开发3D应用。
